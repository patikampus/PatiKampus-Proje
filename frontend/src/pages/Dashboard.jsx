...
const Dashboard = () => {
  ...
  return (
    <div className="w-full max-w-7xl animate-fade-in space-y-6 md:space-y-10 pb-8 md:pb-10">
      ...
      {/* Hero Section */}
      <GlassCard className="p-5 sm:p-8 md:p-12 relative overflow-hidden border-white/10 shadow-2xl" hover={false}>
        {/* Glow Süslemeleri */}
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
          <div className="text-center md:text-left space-y-4 md:space-y-6">
            <Badge variant="purple" className="px-4 py-1.5 sm:px-5 sm:py-2 text-white border-purple-500/40">
            ...
            <p className="text-white/70 text-base sm:text-lg md:text-xl max-w-xl font-medium leading-relaxed italic">
              "Sokaktaki dostlarımızın beslenmesini dijital bir dokunuşla takip edin."
            </p>

            <div className="flex flex-col sm:flex-row sm:flex-wrap justify-center md:justify-start gap-3 sm:gap-5 pt-4 sm:pt-6">
              <Link to="/device-panel" className="w-full sm:w-auto">
                <GlassButton variant="success" className="w-full sm:w-auto px-6 py-3 md:px-10 md:py-4 text-white group shadow-lg shadow-green-500/10">
              ...
              <Link to="/review" className="w-full sm:w-auto">
                <GlassButton variant="primary" className="w-full sm:w-auto px-6 py-3 md:px-10 md:py-4 text-white border-white/20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
        {stats.map((stat, i) => (
          <GlassCard key={i} className="p-5 md:p-8 group relative overflow-hidden">
            <div className="flex justify-between items-start mb-4 md:mb-6 relative z-10">
              <div className={`p-3 md:p-4 rounded-2xl bg-white/5 border border-white/10 ${stat.color} text-2xl md:text-3xl`}>
                {stat.icon}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        ...
        {/* Sistem Akışı */}
        <GlassCard className="lg:col-span-7 p-5 md:p-8" hover={false}>
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <div className="flex items-center gap-3">
              <FiActivity className="text-indigo-400 text-xl md:text-2xl" />
              <h3 className="text-lg md:text-xl font-extrabold text-white tracking-tight">Sistem Akışı</h3>
            </div>
          ...
          <div className="space-y-3 md:space-y-4">
            {[
              ...
              <div key={index} className="flex items-center gap-3 md:gap-5 p-3 md:p-4 bg-white/[0.03] border border-white/5 rounded-2xl">
                <div className="p-2 bg-white/5 rounded-lg text-white shrink-0">{log.icon}</div>
                <div className="flex-grow min-w-0">
                  <p className="text-xs md:text-sm text-white font-bold truncate">{log.text}</p>
                </div>
                <span className="text-[10px] font-bold text-white/30 shrink-0">{log.time}</span>
              </div>
            ))}
          </div>
        </GlassCard>
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 md:gap-6">
          <GlassCard className="p-5 md:p-8 bg-indigo-600/10 border-indigo-500/20" hover={true}>
            <h4 className="text-white font-bold text-lg mb-2">Admin Destek</h4>
            <p className="text-sm text-white/60 mb-4 md:mb-6 leading-relaxed">
